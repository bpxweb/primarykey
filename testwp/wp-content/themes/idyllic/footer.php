...
$footer_column = $idyllic_settings['idyllic_footer_column_section'];
	...
	if ( is_active_sidebar( 'idyllic_footer_options' ) ) :
		...
		<a title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" target="_blank" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo 'www.primarykey.me'; ?></a> | 
						<?php esc_html_e('Designed by:','Primary Key'); ?> <a title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" target="_blank" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_bloginfo( 'name', 'display' ); ?></a><span> 
						<?php  echo '&copy; ' . date_i18n(__('Y','idyllic')).' All right reserverd' ; ?> </span>
					</div>
	<?php endif;
	?>
